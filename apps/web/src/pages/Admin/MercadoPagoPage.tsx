...
export default function MercadoPagoPage() {
    ...
    useEffect(() => {
        ...
        if (!paymentId || pixStatus === 'approved') return;
        ...
        const verify = async () => {
            ...
            if (result.approved) {
                ...
                setPixPaidAt(result.paidAt || new Date().toISOString());
    ...
    }, [generatedPix?.paymentId, pixStatus, checkPixPaymentStatus]);
            ...
            {configured && (
                <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 space-y-4">
                    ...
                    {generatedPix?.qrCode && generatedPix.qrCodeBase64 && (
                        <div className={`grid gap-5 md:grid-cols-[220px_1fr] p-5 rounded-2xl border ${pixStatus === 'approved' ? 'bg-emerald-500/10 border-emerald-400/40' : 'bg-slate-950/60 border-emerald-500/20'}`}>
                            ...
                            <div className="min-w-0 space-y-3">
                                {pixStatus === 'approved' ? <>
                                    ...
                                    <p className="text-sm text-slate-300">O Mercado Pago confirmou o recebimento deste PIX.</p>
                                    {pixPaidAt && <p className="text-xs text-slate-500">Confirmado em {new Date(pixPaidAt).toLocaleString('pt-BR')}</p>}
                                </> : <>
                                    <div className="flex items-center gap-2 text-emerald-400 font-semibold">
                                        <QrCode className="w-5 h-5" /> PIX pronto para pagamento
                                    </div>
                                    <p className="text-xs text-slate-400">Aguardando pagamento. A confirmação será automática.</p>
